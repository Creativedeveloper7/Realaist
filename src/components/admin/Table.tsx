...
export const AdminTable: React.FC<TableProps> = ({ items, onVerify, isDarkMode }) => {
	...
	return (
		<>
			{/* Desktop Table View */}
			<div className={`hidden lg:block overflow-x-auto rounded-xl ${isDarkMode ? 'bg-[#0E0E10] border border-white/10' : 'bg-white border border-gray-200'}`}>
					...
					<tbody>
						{items.map((row) => {
							return (
								<tr 
									key={row.id} 
									className={`text-sm border-b cursor-pointer ${isDarkMode ? 'border-white/10 hover:bg-white/5' : 'border-gray-100 hover:bg-gray-50'}`}
			...
			{/* Mobile Card View */}
			<div className={`lg:hidden space-y-3 ${isDarkMode ? 'bg-[#0E0E10]' : 'bg-white'}`}>
				{items.map((row) => (
					<div
						key={row.id}
						className={`p-4 rounded-xl border cursor-pointer ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}
						onClick={() => handleRowClick(row.id)}
					>
						<div className="flex items-start justify-between gap-3 mb-3">
							<h3 className={`font-semibold text-sm flex-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
								{row.title}
							</h3>
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								<span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Developer</span>
								<span className={`text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{row.developer}</span>
							</div>
							<div className="flex items-center justify-between">
								<span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Verified</span>
								{row.isVerified ? (
									<span className={`text-xs font-medium ${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>Verified</span>
								) : (
									<span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Unverified</span>
							...
							<div className="pt-2">
								<button
									className="w-full px-3 py-2 bg-[#C7A667] text-black rounded-md text-xs font-medium hover:bg-[#B8965A] transition-colors"
									onClick={(e) => {
										e.stopPropagation();
										onVerify(row.id);
									}}
